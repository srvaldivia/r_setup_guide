...
Para instalar la última versión de R ingrese al sitio web de CRAN (*The Comprehensive R Archive Network*) <https://cran.r-project.org/bin/windows/base/>. Una vez allí haga clic en `Download R-4.x.x for Windows` para iniciar la descarga.

A modo de ejemplo, la @fig-1 muestra que la versión disponible al `{r} Sys.Date()` corresponde a la `{r} paste0(R.Version()$major, ".", R.Version()$minor)`.
...
Al abrir el archivo recién descargado (`R-4.x.x-win.exe`) se desplegarán una serie de ventanas que configuran el proceso de instalación de R. Siga las instrucciones que aparecen en las imágenes que se muestran a continuación.
