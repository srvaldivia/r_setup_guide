...
Para instalar la última versión de R ingrese al sitio web de CRAN (*The Comprehensive R Archive Network*) <https://cran.r-project.org/bin/windows/base/>. Una vez allí haga clic en `Download R-4.5.3 for Windows` para iniciar la descarga @fig-1.
...
Después de finalizada la descarga abra el archivo `R-4.5.3-win.exe`. Se desplegarán una serie de ventanas que configuran el proceso de instalación de R. Siga las instrucciones que aparecen en las imágenes que se muestran a continuación.
...
Ingrese al sitio web <https://posit.co/download/rstudio-desktop/> y haga clicen el botón `DOWNLOAD RSTUDIO DESKTOP FOR WINDOWS` (@Fig-11) para descargar la última versión de RStudio. Para descargar la versión compatible con otros sistmas operativos como MacOS o Linux baje hasta la sección `All Installers and Tarballs` y descargue el instalador que corresponda a su sistema operativo.

![Componentes de instalación](img/11_rstudio.png){#fig-11 fig-align="center" width="525"}

<br>

Una vez finalizada la descarga abra el archivo descargado, esto desplegará la ventana de inicio de instalación de RStudio (@Fig-12). Siga las instrucciones que aparecen en las imágenes que se muestran a continuación.
...
![Global options \> General \>Basic](img/17_rs.png){#fig-17 fig-align="center"}

En la misma a la sección `Code` seleccione la pestaña `Display` y marque las siguientes opciones de acuerdo a la (@fig-18).

![Global options \> General \> Basic](img/18_rs.png){#fig-18 fig-align="center"}
...
![Ventana principal de RStudio](img/19_rs.png){#fig-19 fig-align="center"}
